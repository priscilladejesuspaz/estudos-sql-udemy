...
select 
	count(*),
    current_date - birth_date /365 as idade_aproximada, 
...
select 
		max(birth_date) as cliente_mais_velho,
        min(birth_date) as cliente_mais_novo
...
from customers
group by income;
having max(income) as cliente_mais_rico;
